...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    val questionsGroup = postings
      .filter(_.postingType == 1)
      .map(post => (post.id, post))

    val answersGroup = postings
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped
      .values
      .map(pp => (pp.head._1, answerHighScore(pp.map(_._2))))
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    scored
      .filter {
        case (post, _) => post.tags.nonEmpty
      }
      .map {
        case (post, s) =>
          (firstLangInTag(post.tags, langs).get * langSpread, s)
      }
  }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues {
      vs =>
        val size = vs.size
        val commonPoint = vs.map(p => p._1).groupBy(identity).maxBy(_._2.size)

        val langIndex = if (commonPoint._1 == 0) 0 else commonPoint._1 / langSpread
        val langLabel: String = langs(langIndex)
        // most common language in the cluster
        val langPercent: Double = commonPoint._2.size * 1.0 / size * 100
        // percent of the questions in the most common language
        val clusterSize: Int = size
        val sortedVs = vs.toList.sortBy(_._2)
        val medianScore: Int = if (size % 2 != 0) sortedVs(math.floor(size / 2).toInt)._2
        else (sortedVs(size / 2)._2 + sortedVs(size / 2 - 1)._2) / 2
